...
그래서 각 봇은 그대로 두고 **세 기록을 수익률로 환산해 47/20/33으로 합산**한다.

비중 근거: `portfolio_mix.py` 역변동성 배분(2023-06~2026-08, 756일).
셋이 사실상 무상관(|ρ| ≤ 0.03)이라 Sharpe 1.31 → 1.87, MDD −20.3% → −16.0%.
...
# v16 정합성 감사 후 갱신 — 장투를 배포본(무필터)으로 다시 계산한 결과
WEIGHTS = {"장투(KR)": 0.47, "크립토 롱숏": 0.20, "코인봇": 0.33}
...
def main():
    ...
    print("=" * 66)
    print("  젯슨 결합 포트폴리오 (역변동성 비중 47/20/33)")
    print("=" * 66)
    ...
    print("\n  ※ 백테스트 기대치: 연율 32.2%, 변동성 17.2%, Sharpe 1.87, MDD −16.0%")
    print("     포워드가 이에 근접하는지가 판정 기준이다. 최소 수개월 필요.")
    # v19 상관 스트레스: 위 기대치는 **셋이 무상관일 때만** 참이다. 상관이 올라가면
    # 두 이득이 다른 속도로 죽는다 — Sharpe 이득은 ρ≈0.6까지 버티는데
    # **MDD 이득은 ρ≈0.4에서 이미 사라진다**(ρ=0.5에서 결합 −22.3% < 개별 최고 −20.3%).
    print("     ⚠️ MDD −16.0%는 무조건적 기대치가 아니다(v19). 상관이 0.4를 넘으면")
    print("        결합 낙폭이 개별 최고(코인봇 −20.3%)보다 나빠진다. Sharpe 이득은")
    print("        ρ≈0.6까지 버틴다 — 위기에 먼저 깨지는 것은 '덜 흔들린다'는 쪽이다.")
