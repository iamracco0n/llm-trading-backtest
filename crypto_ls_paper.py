...
**규칙(백테스트와 동일하게 고정)**
  ...
  · **청산 가드(v15, 2026-08-16 적용)**: 숏 후보에서 20일 변동성 **상위 10% 제외**.
    청산당하는 것이 곧 최상위 변동성 종목이라, 빼면 알파는 거의 유지되고 청산만 준다
    (검증구간 순수익 +1.06→+1.73%, 청산율 5.30→3.22%).
...
━━━ 2026-08-16 수정 — 배포본에 있던 버그 둘. **둘 다 수익을 부풀리는 쪽이었다.** ━━━
  ① **숏 손익 공식이 인버스 선물 것이었다.** `ent/cur − 1`을 쓰고 있었는데, 검증한
     백테스트와 바이낸스 USDT 무기한선물은 선형이라 `(ent − cur)/ent`가 맞다.
     가격 2배가 되면 실제로는 전액 손실(−100%)인데 −50%로 찍혔다.
  ② **코호트 수익률을 그냥 더하고 있었다.** 코호트 하나는 자본의 1/20이므로
     포트폴리오 수익은 합이 아니라 합÷20이다. 20배로 부풀어 있었다.
  합쳐서 누적 **+38.90%p → +1.35%p**로 정정됐다(2026-08-16, 코호트 6개 시점).
  ⚠️ 이 값은 `portfolio_live.py`가 결합 포트폴리오 계산에 그대로 쓰고 있었으므로,
     8/11~8/16 결합 수치도 같이 틀려 있었다. 상태파일 equity 계열도 소급 정정했다.
...
def build_baskets():
    ...
    # ── v15 청산 가드 ─────────────────────────────────────────────
    # 저변동성 팩터는 "가장 변동성이 큰 것"을 숏하는데, 강제청산은 코인이 2배가 될
    # 때 일어나므로 **청산당하는 것이 정확히 그 최상위 변동성 종목**이다. 숏 후보에서
    # 변동성 상위 10%만 빼면 알파는 거의 안 잃고 청산만 준다. 방향 예측이 아니라
    # 위험 관리라, 이 저장소가 여덟 번 확인한 "LLM은 예측을 못 한다"에 걸리지 않는다.
    # 검증구간(2025-01~) H+20 L=1 실측:
    #     무가드        순수익 +1.06%  청산율 5.30%  숏후보 232개/일
    #     상위 5% 제외        +1.22       3.58            213
    #     상위 10% 제외 ★     +1.73       3.22            195
    #     상위 20% 제외       +1.48       2.88            162
    # 사전 기준(청산율↓ **그리고** 순수익↑)을 5/10/20% 셋 다 통과했다. 10%는 그중
    # 최댓값을 고른 것이라 이득 폭은 다소 과대평가돼 있을 수 있다. 다만 **가드를
    # 건다는 결정 자체는 값을 어디로 잡든 성립**하고, 곡선이 매끄러워(1.22/1.73/1.48)
    # 특정 값에 매달린 결과가 아니다.
    # 기준선은 **거래가능 전체**의 분위수다(숏가능 종목만이 아니라) — v15와 동일.
    VOL_CAP = 0.10
    thr = float((-df["fac"]).quantile(1 - VOL_CAP))
    cand = df[df["shortable"] & (-df["fac"] < thr)]
    n_cap = int((df["shortable"] & (-df["fac"] >= thr)).sum())
    shorts = cand.nsmallest(TOP_N, "fac")["sym"].tolist()

    diag = (f"거래가능 {len(df)}개 / 숏가능 {int(df['shortable'].sum())}개 "
            f"/ 제외: 페그 {n_peg}, 실물토큰 {n_rwa}"
            f" / 청산가드 상위{VOL_CAP:.0%} 컷 {n_cap}개(σ≥{thr*100:.2f}%)")
    ...
    for co in state["cohorts"]:
        ...
        for side, names in (("long", co["long"]), ("short", co["short"])):
            for sym, ent in names.items():
                ...
                if cur is None:
                    continue
                # ⚠️ 숏은 (ent/cur − 1)이 아니라 **(ent − cur)/ent** 다.
                # 전자는 코인마진(인버스) 선물의 수익률이고, 우리가 검증한 백테스트
                # (v6/v15)와 바이낸스 USDT 무기한선물은 **선형**이다. 2026-08-16에
                # 발견해 고쳤다. 틀린 쪽은 **항상 실제보다 좋게** 나온다:
                #   가격 x0.7 → 배포본 +42.9% vs 실제 +30.0%
                #   가격 x1.3 → 배포본 −23.1% vs 실제 −30.0%
                #   가격 x2.0 → 배포본 −50.0% vs 실제 −100.0%(전액 손실)
                # 즉 이익은 부풀고 손실은 깎여 한쪽으로만 편향된다.
                raw = (cur / ent - 1) if side == "long" else (ent - cur) / ent
...
def cmd_run(args):
    ...
    # ⚠️ 코호트 수익률을 **그냥 더하면 안 된다.** 코호트 하나에 들어가는 자본은
    # 전체의 1/20(CAP_PER_COHORT)이므로, 포트폴리오 수익률은 합이 아니라
    # **합 ÷ 20**이다. 그냥 더하면 20배로 부푼다 — 2026-08-16에 발견했다
    # (코호트 6개 평균 +6.5%인데 누적 +38.9%p로 찍히고 있었다. 실제는 +1.94%p).
    # 램프업 구간에서도 이게 맞다: 아직 안 깔린 자본은 현금(수익 0)이고,
    # /20이 그 현금 비중을 정확히 반영한다.
    # 청산 완료분도 같다. `krw`(= CAP_PER_COHORT × ret/100)는 원래 맞게 계산돼
    # 있었으므로, sum(krw)/1,000,000 과 아래 real 이 일치하는지로 검산된다.
    live = sum(c["mark"] for c in state["cohorts"]) / HOLD_DAYS
    real = sum(c["ret"] for c in state["closed"]) / HOLD_DAYS
    print(f"\n  보유 코호트 {len(state['cohorts'])}개 | 평가손익 {live:+.2f}%p "
          f"(코호트 평균 {live*HOLD_DAYS/max(len(state['cohorts']),1):+.2f}%)")
